...
def _slim_logs_enabled() -> bool:
    """Return whether high-frequency task progress prints should be muted."""
...
def _extract_trace_type_from_raw_prompt(raw_content: Any) -> str:
    """Extract `[TraceType] <type>` marker from prompt raw_content."""
    ...
    for item in raw_content:
        ...
        for line in content.splitlines():
            stripped = line.strip()
            if stripped.lower().startswith("[tracetype]"):
...
class AgentModeDaemon:
    ...
    def __init__(
        ...
        if mode == "v0":
            ...
            if llm_proxy is None:
                enable_message_inspection = os.getenv("AGL_LLM_PROXY_MESSAGE_INSPECTION", "").lower() in {
                    "1",
                    "true",
                    "yes",
                    "on",
                }
                disable_stream_conversion = os.getenv("AGL_LLM_PROXY_DISABLE_STREAM_CONVERSION", "").lower() in {
                    "1",
                    "true",
                    "yes",
                    "on",
                }
                proxy_middlewares: List[str] = ["rollout_attempt"]
                if not disable_stream_conversion:
                    proxy_middlewares.append("stream_conversion")
                if enable_message_inspection:
                    proxy_middlewares.append("message_inspection")
                self.llm_proxy = LLMProxy(
                    port=_find_available_port(),
                    model_list=[],
                    store=store,
                    middlewares=proxy_middlewares,
                )
            else:
    ...
    async def _update_proxy_server_v1(self):
        ...
        print(f"LLMProxy access endpoint: {self.llm_proxy.server_launcher.access_endpoint}/v1")
        print(f"LLMProxy backend servers: {self.backend_llm_server_addresses}")
    ...
    def _validate_data(self, rollout: RolloutLegacy):
        ...
        # Debug: print triplet info
        if rollout.triplets and not _slim_logs_enabled():
            print(f"[DEBUG] Rollout {rollout.rollout_id} has {len(rollout.triplets)} triplets")
            for i, triplet in enumerate(rollout.triplets[:3]):  # Only print first 3
                prompt_len = len(triplet.prompt.get("token_ids", []))
                resp_len = len(triplet.response.get("token_ids", []))
                print(f"  Triplet {i}: prompt_tokens={prompt_len}, response_tokens={resp_len}")
                if resp_len == 0:
                    print(f"    WARNING: Empty response! Raw response: {triplet.response}")

        if rollout.triplets and any(not r.response.get("token_ids", []) for r in rollout.triplets):
            empty_response_count = sum(1 for r in rollout.triplets if not r.response.get("token_ids", []))
            print(
                f"Warning: Rollout {rollout.rollout_id} contains {empty_response_count} triplets with empty response."
            )
        elif rollout.triplets and any(not r.prompt.get("token_ids", []) for r in rollout.triplets):
            empty_prompt_count = sum(1 for r in rollout.triplets if not r.prompt.get("token_ids", []))
            print(f"Warning: Rollout {rollout.rollout_id} contains {empty_prompt_count} triplets with empty prompt.")
    ...
    async def _validate_data_v1(self, rollout: Rollout) -> RolloutLegacy:
        ...
        # Extract final reward from triplets (legacy, for backward compatibility)
        ...
        # Extract per-triplet rewards (new: support different reward per triplet)
        triplet_rewards: List[Optional[float]] = []
        if triplets:
            for triplet in triplets:
                triplet_rewards.append(triplet.reward)
        ...
        result_rollout = RolloutLegacy(
            ...
            triplets=triplets,
            metadata=rollout.metadata or {},
            triplet_rewards=triplet_rewards,  # new field
        )
    ...
    def get_test_metrics(self):
        ...
        for rollout_id, rollout in self._completed_rollouts_v0.items():
            ...
            final_reward = self._rollout_mean_reward(rollout)
    ...
    def get_train_data_batch(
        ...
        assert len(self._completed_rollouts_v0) == self._total_tasks_queued
        allowed_trace_types_env = os.environ.get("AGL_ALLOWED_TRACE_TYPES", "")
        allowed_trace_types = {t.strip().lower() for t in allowed_trace_types_env.split(",") if t.strip()}
        ...
        for rollout_id, rollout in self._completed_rollouts_v0.items():
            ...
            trace_list = [
                {
                    ...
                    "trace_type": _extract_trace_type_from_raw_prompt(t.prompt.get("raw_content", [])),
                }
            ...
            info = {
                ...
                "triplet_rewards": rollout.triplet_rewards,  # new: per-triplet rewards
                "triplet_metadata": [t.metadata if t.metadata is not None else {} for t in rollout.triplets],
            }
        ...
        rollout_reward_values: Dict[str, List[float]] = defaultdict(list)
        data_id_list: List[str] = []
        rollout_id_list: List[str] = []
        turn_index_list: List[int] = []
        character_name_list: List[str] = []
        short_term_reward_list: List[float] = []
        long_term_reward_list: List[float] = []
        combine_short_weight_list: List[float] = []
        combine_long_weight_list: List[float] = []
        ...
        if self.trace_aggregator.get("level", "transition") == "transition":
            for rollout_id, sample_info in finished_id_to_sample_info.items():
                triplet_rewards = sample_info.get("triplet_rewards", [])
                triplet_metadata = sample_info.get("triplet_metadata", [])
                for turn_index, trace in enumerate(sample_info["trace_list"]):

                    prompt_ids, response_ids = trace["prompt_ids"], trace["response_ids"]
                    trace_type = str(trace.get("trace_type", "")).strip().lower()

                    # Optional type whitelist: keep only selected trace types.
                    if allowed_trace_types and trace_type not in allowed_trace_types:
                        if not _slim_logs_enabled():
                            print(
                                f"[DEBUG] Skipping trace {turn_index} with trace_type='{trace_type or 'unknown'}' in rollout {rollout_id}"
                            )
                        continue

                    # Skip traces with empty response_ids (e.g., non-vLLM evaluator calls)
                    if len(response_ids) == 0:
                        if not _slim_logs_enabled():
                            print(f"[DEBUG] Skipping trace {turn_index} with empty response_ids in rollout {rollout_id}")
                        continue

                    # Use per-triplet reward if available, otherwise use fallback reward
                    if turn_index < len(triplet_rewards) and triplet_rewards[turn_index] is not None:
                        step_reward = float(triplet_rewards[turn_index])
                    else:
                        # Fallback to rollout-level reward when triplet reward is missing
                        step_reward = float(sample_info["reward"])
                    reward_list.append(step_reward)
                    rollout_reward_values[rollout_id].append(step_reward)
                    ...
                    metadata = triplet_metadata[turn_index] if turn_index < len(triplet_metadata) else {}
                    character_name_list.append(str(metadata.get("agl.roleplay.character_name", "")))
                    short_term_reward_list.append(float(metadata.get("agl.roleplay.short_term_reward", reward_list[-1])))
                    long_term_reward_list.append(float(metadata.get("agl.roleplay.long_term_reward", sample_info["reward"])))
                    combine_short_weight_list.append(float(metadata.get("agl.roleplay.short_term_weight", 0.50)))
                    combine_long_weight_list.append(float(metadata.get("agl.roleplay.long_term_weight", 0.50)))
            ...
            for rollout_id, sample_info in finished_id_to_sample_info.items():
                ...
                for turn_index, trace in enumerate(sample_info["trace_list"]):
                    ...
                    if allowed_trace_types and trace_type not in allowed_trace_types:
                        if not _slim_logs_enabled():
                            print(
                                f"[DEBUG] Skipping trace {turn_index} with trace_type='{trace_type or 'unknown'}' in rollout {rollout_id} (trajectory mode)"
                            )
                        continue

                    # Skip traces with empty response_ids (e.g., non-vLLM evaluator calls)
                    if len(trace["response_ids"]) == 0:
                        if not _slim_logs_enabled():
                            print(
                                f"[DEBUG] Skipping trace {turn_index} with empty response_ids in rollout {rollout_id} (trajectory mode)"
                            )
                        continue
                ...
                for current_merged_trace_idx in merged_trace_idx:
                    ...
                    # Use the reward of the last triplet in the merged segment
                    triplet_rewards = sample_info.get("triplet_rewards", [])
                    if current_merged_trace_idx and triplet_rewards:
                        # Use the last triplet's reward
                        last_triplet_idx = current_merged_trace_idx[-1]
                        if last_triplet_idx < len(triplet_rewards) and triplet_rewards[last_triplet_idx] is not None:
                            step_reward = float(triplet_rewards[last_triplet_idx])
                        else:
                            step_reward = float(sample_info["reward"])
                    else:
                        step_reward = float(sample_info["reward"])
        ...
        if rollout_reward_values:
            weighted_sum = 0.0
            weighted_count = 0
            for rewards in rollout_reward_values.values():
                if not rewards:
                    continue
                reward_arr = np.asarray(rewards, dtype=np.float32)
                weighted_sum += float(np.mean(reward_arr)) * int(reward_arr.shape[0])
                weighted_count += int(reward_arr.shape[0])
            weighted_rollout_mean = weighted_sum / max(1, weighted_count)
            all_triplet_mean = float(np.mean(np.asarray(reward_list, dtype=np.float32))) if reward_list else weighted_rollout_mean
        else:
            weighted_rollout_mean = float(np.mean(list(finished_id_to_final_reward.values())))
            all_triplet_mean = weighted_rollout_mean

        if n_transition == 0:
            data_metrics = {
                "training/reward": weighted_rollout_mean,
                "training/reward_all_triplets": all_triplet_mean,
        ...
        if self.trace_aggregator.get("level", "transition") == "transition":
            ...
            data_proto.non_tensor_batch["character_name_list"] = np.array(character_name_list, dtype=object)  # type: ignore
            data_proto.non_tensor_batch["short_term_reward_list"] = np.array(short_term_reward_list, dtype=np.float32)  # type: ignore
            data_proto.non_tensor_batch["long_term_reward_list"] = np.array(long_term_reward_list, dtype=np.float32)  # type: ignore
            data_proto.non_tensor_batch["combine_short_weight_list"] = np.array(combine_short_weight_list, dtype=np.float32)  # type: ignore
            data_proto.non_tensor_batch["combine_long_weight_list"] = np.array(combine_long_weight_list, dtype=np.float32)  # type: ignore
    ...
    def _rollout_mean_reward(self, rollout: RolloutLegacy) -> float:
        """Return rollout reward as the mean of per-triplet rewards when available.

        Validation metrics should reflect the rollout-wide average reward, which matches
        the `ROLLOUT REWARD STATS mean=...` printed by the roleplay example. Fall back to
        the legacy rollout-level reward only when no triplet rewards are available.
        """
        reward_values: List[float] = []

        if rollout.triplet_rewards:
            reward_values.extend(float(reward) for reward in rollout.triplet_rewards if reward is not None)

        if not reward_values and rollout.triplets:
            reward_values.extend(float(triplet.reward) for triplet in rollout.triplets if triplet.reward is not None)

        if reward_values:
            return float(np.mean(np.asarray(reward_values, dtype=np.float32)))

        return float(self._fillna_reward(rollout))
