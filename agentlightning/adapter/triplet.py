...
class LlmProxyTraceToTriplet(TraceToTripletBase):
    ...
    def _reward_metadata_from_span(self, span: Span) -> Dict[str, Any]:
        """Extract custom reward metadata attached to reward spans."""
        attrs = span.attributes or {}
        metadata: Dict[str, Any] = {}
        for key in [
            "agl.roleplay.character_name",
            "agl.roleplay.short_term_reward",
            "agl.roleplay.long_term_reward",
            "agl.roleplay.combined_reward",
            "agl.roleplay.short_term_weight",
            "agl.roleplay.long_term_weight",
        ]:
            if key in attrs:
    ...
    def adapt(self, source: Sequence[Span], /) -> List[Triplet]:  # type: ignore
        ...
        rewards: List[Tuple[int, Optional[float], Dict[str, Any]]] = []
        for s in spans:
            val = self._maybe_reward_value(s)
            if val is not None:
                rewards.append((s.sequence_id, val, self._reward_metadata_from_span(s)))
        ...
        for r_seq, r_val, r_meta in sorted(rewards, key=lambda x: x[0]):
            for item in reversed(llm_items):
                sid = item["span"].span_id
                if sid in assigned:
                    continue
                if item["seq"] < r_seq:
                    assigned[sid] = {"reward": r_val, "metadata": r_meta}
                    break
        ...
        for item in llm_items:
            ...
            triplets.append(
                Triplet(
                    ...
                    reward=cast(Optional[float], assigned.get(s.span_id, {}).get("reward")),
                    metadata=dict(
                        # This is called response_id to align with the other adapters.
                        response_id=item["request_id"],
                        **cast(Dict[str, Any], assigned.get(s.span_id, {}).get("metadata", {})),
                    ),
                )
            )
